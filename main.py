...
links = soup.find_all('li', {"class":"product-category product"})
...
for i in links:
    ...
    for j in links_items:
        ...
        # price = 'test'
        description = cleanhtml(str(soup.find('div', {"class":"woocommerce-Tabs-panel--description"})))
        # description = 'test'
        img = extract_src(str(soup.find('figure', {"class":"woocommerce-product-gallery__wrapper"})))
        # img = 'test'
        ...
        data[cleanhtml(str(i))][cleanhtml(str(title))] = {"title":title, "price": price, "description": description, "img": img}
